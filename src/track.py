...
class Track:
    ...
    def get_s(self):
        points = ((self.x_points[1:] - self.x_points[0:-1])**2 + (self.y_points[1:] - self.y_points[0:-1])**2)**0.5
    ...
    def get_proj(self,state):
        cur_pos = [state.X, state.Y]
        dists  = (self.x_points - cur_pos[0])**2 + (self.y_points - cur_pos[1])**2
