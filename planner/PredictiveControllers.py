...
class LMPC():
    ...
    """
    def __init__(self, numSS_Points, numSS_it, QterminalSlack, mpcParameters, predictiveModel, dt = 0.1):
        """Initialization
        Arguments:
        ...
        self.zt = np.array([0.0, 0.0, 0.0, 0.0, 150.0, 0.0])
    ...
    def addTerminalComponents(self,x0):
        ...
        Arguments:
            x: initial condition
        """
        # Update zt and xLin if they have crossed the finish line. We want s \in [0, TrackLength]
        if (self.zt[4]-x0[4] > self.predictiveModel.TrackLength/2): # look at this condition again
            self.zt[4] = np.max([self.zt[4] - self.predictiveModel.TrackLength,0])
            self.xLin[-1,4] = self.xLin[-1,4]- self.predictiveModel.TrackLength
    ...
    def addTrajectory(self, x, u, x_glob):
        ...
        Arguments:
            ...
            u: applied inputs
            x_glob: closed-loop trajectory in global frame
        """
    ...
    def computeCost(self, x, u):
        ...
        for i in range(0, x.shape[0]):
            ...
            elif x[x.shape[0] - 1 - i, 4]< self.predictiveModel.TrackLength:
    ...
    def addPoint(self, x, u):
        ...
        Arguments:
            ...
            u: current input
        """
        self.SS[self.it - 1]  = np.append(self.SS[self.it - 1], np.array([x + np.array([0, 0, 0, 0, self.predictiveModel.TrackLength, 0])]), axis=0)
    ...
    def selectPoints(self, it, zt, numPoints):
        """select (numPoints)-nearest neivbor to zt. These states will be used to construct the safe set and the value function approximation
        Arguments:
            x: current state
            u: current input
        """
        ...
        if self.xPred == []:
            Sel_Qfun = self.Qfun[it][indexSSandQfun]
        elif (np.all((self.xPred[:, 4] > self.predictiveModel.TrackLength) == False)):
            Sel_Qfun = self.Qfun[it][indexSSandQfun]
        elif it < self.it - 1:
            Sel_Qfun = self.Qfun[it][indexSSandQfun] + self.Qfun[it][0]
        else:
            sPred = self.xPred[:, 4]
            predCurrLap = self.N - sum(sPred > self.predictiveModel.TrackLength)
    ...
    def solve(self, x0):
        ...
        self.xLin[:,4] = np.abs(self.xLin[:,4])
